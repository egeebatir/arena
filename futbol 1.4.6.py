...
TABLE_BG = (255, 255, 255, 220) # White, slightly transparent

# --- SES AYARLARI ---
STADIUM_FILE = "stadium.mp3"
COLLISION_FILE = "collision1.wav" 
RED_CARD_FILE = "whistle.wav" # NEW SOUND FILE
...
# FONTS
font_score = pygame.font.SysFont("Arial", 60, bold=True) # Bigger score font
font_timer = pygame.font.SysFont("Arial", 30, bold=True)
font_event = pygame.font.SysFont("Arial", 60, bold=True)
font_team = pygame.font.SysFont("Arial", 28, bold=True) # Bigger team font
...
font_notification = pygame.font.SysFont("Arial", 32, bold=True) # Notification font
...
class Ball:
    ...
    def draw(self, surface):
        ...
        # Draw outline
        pygame.draw.circle(ball_surf, BLACK, (self.radius, self.radius), self.radius, 2)
...
goll_timer = 0
# Modified to store Frame as 4th element: (Time, Type, ScorerName, Frame)
goal_events_1 = [] 
goal_events_2 = [] 
state = "MENU"
frame_counter = 0
...
end_match_timer = 0
halftime_timer = 0 # NEW: for halftime display
...
def start_match():
    ...
    global red_card_obj, red_card_spawned_this_half, halftime_timer
...
while running:
    ...
    if state == "MENU":
                ...
                if not red_card_obj.active:
                    ...
                    for b in [ball1, ball2]:
                        if red_card_obj.check_collision(b):
                            ...
                            # STORE EVENT AS TUPLE: (Time, Type, ScorerName, Frame)
                            time_mark = f"{sim_minute}'"
                            if b == ball1:
                                goal_events_1.append((time_mark, "RED_CARD", "", frame_counter))
                            else:
                                goal_events_2.append((time_mark, "RED_CARD", "", frame_counter))
                ...
                if dist > ARENA_RADIUS - b.radius - 5:
                    ...
                    if diff < GOAL_WIDTH_RADIANS / 2:
                        ...
                        if b == ball1:
                            ...
                            # STORE EVENT AS TUPLE: (Time, Type, ScorerName, Frame)
                            goal_events_1.append((time_mark, "GOAL", scorer_name, frame_counter))
                            ...
                            # STORE EVENT AS TUPLE: (Time, Type, ScorerName, Frame)
                            goal_events_2.append((time_mark, "GOAL", scorer_name, frame_counter))
        ...
        panel_w, panel_h = 480, 100 # Bigger table
        panel_x = WIDTH // 2 - panel_w // 2
        
        # Transparent White Background
        table_surf = pygame.Surface((panel_w, panel_h), pygame.SRCALPHA)
        table_surf.fill(TABLE_BG)
        screen.blit(table_surf, (panel_x, 20))
        
        # Border
        pygame.draw.rect(screen, WHITE, (panel_x, 20, panel_w, panel_h), 3, border_radius=10)
        
        # Team Names (Leveled with score)
        ts_surf = font_team.render(TEAMS[TEAM_NAMES[selected_home_idx]]["short"], True, team1_colors[0])
        fb_surf = font_team.render(TEAMS[TEAM_NAMES[selected_away_idx]]["short"], True, team2_colors[0])
        
        # Positioning Names
        screen.blit(ts_surf, (panel_x + 30, 45)) 
        screen.blit(fb_surf, (panel_x + panel_w - fb_surf.get_width() - 30, 45))
        
        # Scores (Colored)
        s1_surf = font_score.render(str(score1), True, team1_colors[1])
        s2_surf = font_score.render(str(score2), True, team2_colors[1])
        dash_surf = font_score.render("-", True, BLACK)
        
        center_score_x = WIDTH // 2
        screen.blit(s1_surf, (center_score_x - 60, 30))
        screen.blit(dash_surf, (center_score_x - dash_surf.get_width()//2, 30))
        screen.blit(s2_surf, (center_score_x + 30, 30))

        # Time Display (Inside table, below scores)
        time_str = ""
        ...
        # 10 game minutes = 10 * FRAMES_PER_SIM_MINUTE frames
        notif_duration = 10 * FRAMES_PER_SIM_MINUTE
        ...
        def draw_event_list(events_list, x_pos, align_right=False):
            ...
            for event_data in visible_events:
                t_str, e_type, scorer, ev_frame = event_data
                
                # Check if this event is new (less than 10 mins old)
                is_new = (state in ["FIRST_HALF", "SECOND_HALF"]) and (frame_counter - ev_frame < notif_duration)
                
                if is_new:
                    # It will be drawn big in the center, skip drawing in list or draw faded?
                    # Prompt says "visible big... then they become smaller and sit wherever it normally be"
                    # So we skip drawing it in the small list for now
                    if e_type == "GOAL":
                        txt = f"GOAL! {scorer} ({t_str})"
                        col = GOLD
                    else:
                        txt = f"RED CARD! ({t_str})"
                        col = RED_CARD_COLOR
        ...
        # Draw BIG notifications
        notif_y = 150
        if recent1:
            txt, col = recent1
            nsurf = font_notification.render(txt, True, col)
            screen.blit(nsurf, (WIDTH//2 - nsurf.get_width()//2, notif_y))
            notif_y += 40
