...
pygame.display.set_caption("Futbol Simulasyonu - Red Card Final")
...
goal_rot_speed = 0.02
goll_timer = 0
goal_events_1 = [] # Stores tuples: (Time, Type)
goal_events_2 = [] # Stores tuples: (Time, Type)
state = "MENU"
frame_counter = 0
    ...
    if state == "MENU":
        ...
        if state in ["FIRST_HALF", "SECOND_HALF"]:
            ...
            if state == "FIRST_HALF":
                ...
                # --- RED CARD SPAWN LOGIC (FIRST HALF 10%) ---
                if not red_card_spawned_this_half and not red_card_obj:
                    if random.random() < 0.1 / (45 * 60) * 10: 
                ...
                # --- RED CARD SPAWN LOGIC (SECOND HALF 30%) ---
                if not red_card_spawned_this_half and not red_card_obj:
                    if random.random() < 0.3 / (45 * 60) * 10: 
                        red_card_obj = RedCard(center_x, center_y)
                        red_card_spawned_this_half = True
                ...
                if not red_card_obj.active:
                    ...
                    for b in [ball1, ball2]:
                        if red_card_obj.check_collision(b):
                            ...
                            # STORE EVENT AS TUPLE: (Time, Type)
                ...
                if dist > ARENA_RADIUS - b.radius - 5:
                    ...
                    if diff < GOAL_WIDTH_RADIANS / 2:
                        ...
                        if b == ball1:
                            score1 += 1
                            # STORE EVENT AS TUPLE: (Time, Type)
                            goal_events_1.append((time_mark, "GOAL"))
                            ball1.x, ball1.y = center_x, center_y
                            play_goal_music_for_team(team1_name)
                        else:
                            score2 += 1
                            # STORE EVENT AS TUPLE: (Time, Type)
                            goal_events_2.append((time_mark, "GOAL"))
        ...
        for event_data in goal_events_1[-5:]:
            t_str, e_type = event_data # Unpack tuple
