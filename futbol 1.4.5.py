...
def load_player_database():
    ...
    # 2. Combine it with the filename
    file_path = os.path.join(base_path, "players.txt")
    
    print(f"Looking for players.txt at: {file_path}") # DEBUG PRINT
        ...
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                for line in f:
                    if ":" in line:
                        parts = line.split(":")
                        # Clean up keys to match TEAMS dictionary exactly
                        ...
                        if p_names:
                            db[t_name] = p_names
            print(f"SUCCESS: Loaded {len(db)} teams from database.") # DEBUG PRINT
...
def get_random_player_name(team_key):
    # Try to find team in database (Handle Turkish chars and casing)
    # This is a basic fuzzy match or direct match
    if team_key in PLAYER_DATABASE:
        players = PLAYER_DATABASE[team_key]
        if players:
            return random.choice(players)
    
    # Fallback if no file or team not found
...
def play_collision_sound():
    if collision_sound:
        # Prevent spamming: only play if not already playing or overlapping minimally
...
goll_timer = 0
goal_events_1 = [] # Stores tuples: (Time, Type, ScorerName)
goal_events_2 = [] # Stores tuples: (Time, Type, ScorerName)
state = "MENU"
frame_counter = 0
...
def start_match():
    ...
    global red_card_obj, red_card_spawned_this_half
...
while running:
    ...
    if state == "MENU":
                ...
                if not red_card_obj.active:
                    ...
                    for b in [ball1, ball2]:
                        if red_card_obj.check_collision(b):
                            ...
                            # STORE EVENT AS TUPLE: (Time, Type, ScorerName)
                            time_mark = f"{sim_minute}'"
                            if b == ball1:
                                goal_events_1.append((time_mark, "RED_CARD", ""))
                            else:
                                goal_events_2.append((time_mark, "RED_CARD", ""))
                ...
                if dist > ARENA_RADIUS - b.radius - 5:
                    ...
                    if diff < GOAL_WIDTH_RADIANS / 2:
                        ...
                        if b == ball1:
                            ...
                            # STORE EVENT AS TUPLE: (Time, Type, ScorerName)
                            goal_events_1.append((time_mark, "GOAL", scorer_name))
                            ...
                            # STORE EVENT AS TUPLE: (Time, Type, ScorerName)
                            goal_events_2.append((time_mark, "GOAL", scorer_name))
        ...
        # --- DRAW STATS / EVENTS (UPDATED) ---
        y_off = 110
        for event_data in goal_events_1[-5:]:
            t_str, e_type, scorer = event_data # Unpack tuple
            if e_type == "GOAL":
            ...
            if e_type == "RED_CARD":
                rc_rect = pygame.Rect(panel_x + 30 + t_surf.get_width() + 5, y_off + 2, 10, 14)
                pygame.draw.rect(screen, RED_CARD_COLOR, rc_rect)
                pygame.draw.rect(screen, WHITE, rc_rect, 1)
            y_off += 20
            
        y_off = 110
        for event_data in goal_events_2[-5:]:
            t_str, e_type, scorer = event_data # Unpack tuple
            if e_type == "GOAL":
                display_str = f"{t_str} {scorer}"
            else:
            ...
            if e_type == "RED_CARD":
                # Draw symbol to the right of text
                rc_rect = pygame.Rect(panel_x + panel_w - 50 + t_surf.get_width() + 5, y_off + 2, 10, 14)
                pygame.draw.rect(screen, RED_CARD_COLOR, rc_rect)
                pygame.draw.rect(screen, WHITE, rc_rect, 1)
            y_off += 20
